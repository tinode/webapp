  ...
  switch (style) {
    ...
    default:
      if (!el) {
        ...
        values = [<i key="x0" className="m-icon gray">extension</i>, ' '].concat(body);
      }
      break;
  }
  if (!el) {
...
export function previewFormatter(style, data, values, key) {
  ...
  switch (style) {
    case 'AU':
      ...
      el = React.Fragment;
      values = [<i key="au" className="m-icon thin">mic</i>, ' ', secondsToTime(data.duration/1000)];
      break;
    case 'BR':
    ...
    case 'IM':
      ...
      el = React.Fragment;
      values = [<i key="im" className="m-icon thin">photo</i>, ' ', this.formatMessage(messages.drafty_image)];
      break;
    case 'BN':
      el = 'span';
    ...
    case 'FM':
      el = React.Fragment;
      values = [<i key="fm" className="m-icon thin">browse</i>,
    ...
    case 'EX':
      ...
      el = React.Fragment;
      values = [<i key="ex" className="m-icon thin">attachment</i>, ' ', this.formatMessage(messages.drafty_attachment)];
      break;
    case 'VC':
      el = CallStatus;
      if (data) {
    ...
    case 'VD':
      ...
      el = React.Fragment;
      values = [<i key="im" className="m-icon thin">play_circle_outline</i>, ' ', this.formatMessage(messages.drafty_video)];
      break;
    default:
      if (!el) {
        // Unknown element.
        el = React.Fragment;
        values = [<i key="x0" className="m-icon gray thin">extension</i>, ' ', this.formatMessage(messages.drafty_unknown)];
      }
      break;
  }
  if (!el) {
  ...
  if (['BR', 'EX', 'IM', 'MN', 'VD'].includes(style)) {
    ...
    switch(style) {
      ...
      case 'EX':
        ...
        el = React.Fragment;
        values = [<i key="ex" className="m-icon">attachment</i>,
  ...
  if (style == 'IM' || style == 'VD') {
    ...
    if (stack.includes('QQ')) {
      ...
      return React.createElement('span', {key: key},
        [<i key="qq" className="m-icon">format_quote</i>, ' ']);
    }
