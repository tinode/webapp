...
Topic.prototype = {
  ...
  _updateDeletedRanges: function() {
    ...
    // DEBUG: list prior gaps.
    let gaps = [];
    this._messages.forEach((data) => {
      if (data.hi || data._status == MESSAGE_STATUS_DEL_RANGE) {
        gaps.push(data);
      }
    });
    if (gaps.length > 0) {
    ...
    this._messages.filter((data) => {
      ...
      if (data.seq == (prev.hi || prev.seq) + 1) {
        ...
        if (data.hi && prev.hi) {
          ...
          console.log("extend_1", prev, data);
          prev.hi = data.hi;
          return false;
        }
        prev = data;
      ...
      // Check if the previous is also a gap marker, alter it instead of creating a new one.
      if (prev.hi) {
        console.log("extend_2", prev, data.seq, data.hi);
    ...
    if (ranges.length > 0) {
      console.log("new deleted ranges", ranges);
    }
