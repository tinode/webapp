...
class AudioRecorder extends (react__WEBPACK_IMPORTED_MODULE_0___default().PureComponent) {
  ...
  initMediaRecording(stream) {
    ...
    this.mediaRecorder.onstop = _ => {
      if (this.durationMillis > MIN_DURATION) {
        this.getRecording().then(result => this.props.onFinished(result.url, result.preview, this.durationMillis));
      } else {
  ...
  getRecording() {
    ...
    return blob.arrayBuffer().then(ab => this.audioContext.decodeAudioData(ab)).then(data => this.createPreview(data)).then(preview => {
      return {
        url: url,
