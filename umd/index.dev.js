...
class MessagesView extends (react__WEBPACK_IMPORTED_MODULE_0___default().Component) {
  ...
  handlePickReply(seq, content, forwarded, senderId, senderName) {
    ...
    if (Drafty.isValid(content)) {
      content = Drafty.preview(content, _config_js__WEBPACK_IMPORTED_MODULE_14__.QUOTED_REPLY_LENGTH, undefined, forwarded != null);
    } else {
...
class ForwardMenu extends (react__WEBPACK_IMPORTED_MODULE_0___default().Component) {
  ...
  handleSearchContacts(query) {
    this.setState({
      query: tinode_sdk__WEBPACK_IMPORTED_MODULE_1___default().isNullValue(query) ? null : query
    });
    this.props.onSearchContacts(query);
  }
