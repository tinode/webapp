...
var MessagesView = function (_React$Component) {
  ...
  _createClass(MessagesView, [{
    ...
    key: "propsChange",
    value: function propsChange(nextProps, prevState) {
      var _this2 = this;

      var nextState = {};

      if (!nextProps || !nextProps.topic) {
        var oldTopic = prevState.topic;
        this.setState({
          messages: [],
          onlineSubs: [],
          topic: null
      ...
      var newGroupTopic = tinode_sdk__WEBPACK_IMPORTED_MODULE_2___default.a.isNewGroupTopicName(nextProps.topic);
      var tryToResubscribe = !nextProps.connected && nextProps.connected;
      ...
      if (!topic.isSubscribed() && tryToResubscribe) {
        ...
        var setQuery = newGroupTopic ? nextProps.newGroupTopicParams : undefined;
        this.setState({
        ...
        topic.subscribe(getQuery.build(), setQuery).then(function (ctrl) {
          ...
          var formatMessage = nextProps.intl.formatMessage;
      ...
      switch (action) {
        case 'accept':
          response = topic.setMeta({
            sub: {
              mode: topic.getAccessMode().getGiven()
            }
          });
          break;
